...
Each dimension leverages a group of the opportunities from Table~\ref{tab:dependency-signals}
% Section~\ref{sec:motivating-examples}
: refactoring restructures the DAG to share or remove work (opportunities~(1)--(4)), materialization decides which results are persisted and composes overlapping rewrites (opportunity~(5) and global rewrite composition), and frequency aligns each node's execution rate with how often its inputs change (opportunity~(6)). 
