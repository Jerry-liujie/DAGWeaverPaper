...
Candidate identification finds where in the DAG the refactoring opportunities~(1)--(4) of Section~\ref{sec:motivating-examples} are worth pursuing. Refactoring reasons jointly about a region of interdependent models, and a rewrite spanning many of them is hard to keep correct; \tool{} therefore works on small, bounded regions rather than the whole project. Because most regions are not worth refactoring, it scans the whole DAG with cheap structural analyses that rank regions by likely benefit, reserving the expensive refactoring step (\S\ref{sec:llm-refactoring}) for the few that pay off.
