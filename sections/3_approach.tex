...
This stage realizes optimization 5 of Table~\ref{tab:dependency-signals}, rewrite-materialization co-optimization, and additionally composes overlapping rewrite candidates into a globally compatible set. 
...
To score a candidate, \tool{} must find its best materialization: the assignment of each node to a table or a view that makes the whole pipeline cheapest to run, where \emph{cost} is the total slot time of one execution. That lowest cost becomes the candidate's score.
