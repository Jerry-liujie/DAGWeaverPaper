...
\node[freqstage, below=2mm of s2.south,
  ...
  {Frequency profile\\\scriptsize(\tofix{source update rates + output demand,
...
Candidate identification finds where in the DAG \tofix{the four graph-refactoring optimizations from Section~\ref{sec:dependencies-as-signals}} are worth pursuing.
...
To address such challenges, DAGSmith matches on the operation each model computes through dataflow analysis, resolving references so that a model boundary no longer hides shared work, and reduces the two to a single reuse candidate, which the LLM and a later \tofix{data-backed equivalence check} confirm is safe to share. It further performs two analyses: reuse finds repeated expensive work to share \tofix{(non-local semantic reuse)}, and prune finds work that no downstream model consumes or that runs too early \tofix{(dependency-edge simplification, downstream-aware pruning, and pipeline-aware work placement)}. Only the highest-scoring regions are passed to the LLM.
...
This stage produces the refactorings that realize \tofix{the four graph-refactoring optimizations from Section~\ref{sec:dependencies-as-signals}}. As motivated in the overview, producing these cross-boundary, semantic rewrites is a challenging reasoning task, which is why \tool{} uses state-of-the-art frontier LLMs rather than a fixed rule set.
...
This stage realizes \tofix{rewrite-materialization co-optimization}. 
A refactoring's benefit cannot be determined in isolation: whether it helps depends on which of its nodes are stored as tables and which are inlined as views, and the best choice differs from one candidate to the next, so a candidate must be scored together with its own best materialization. \tofix{This is especially important when a rewrite creates additional reusable output: the richer rewrite may be locally more expensive, but it can become beneficial when the output is materialized once and reused by enough downstream consumers.} Even then, a second challenge remains: candidates overlap and cannot all be adopted at once, so they must be composed into a compatible set. This subsection addresses both (Figure~\ref{fig:materialization}): tuning one candidate's materialization, then \tofix{separately} selecting a compatible set.
