...
\emph{CSE} is a cross-model common subexpression baseline~\cite{silva2012cse}, \linc{broken sitation.} \linc{citation still broken when I compile it...} which detects subexpressions shared across a SCOPE script's stages and materializes each once, re-optimized at the query optimizer's memo. The memo step is not portable, since managed warehouses do not expose their optimizer's internal state to external tools. We adapt the approach by detecting shared subexpressions at the AST level and selecting which to materialize heuristically. CSE is the strongest available position for a syntactic, cross-model approach on a managed warehouse. \linc{is ``strongest available'' the correct way to phrase this? ``strongest'' according to what standard?}
